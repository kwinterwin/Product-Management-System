...
let usersData = {
	...
	login(req, res) {
		...
		let query = `select p.*, us.surname from prms.user_logindata as p, prms.users as us where login='${user.login}' 
						and password='${user.password}' and us.user_id=p.id`;
		con.con.query(query, (err, result) => {
			...
			if (!result || result.length == 0) {
				query = `select * from prms.user_logindata where login='${user.login}' and password='${user.password}'`;
				con.con.query(query, (err, result) => {
					if (err) {
						res.status(500).send(err);
						console.log(err);
